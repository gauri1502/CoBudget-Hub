...
const groupDAO = require("../Group/group.controller");
...
exports.addExpense = async (req, res) => {
  try {
    ...
    console.log(expense.expenseOwner);
...
exports.editExpense = async (req, res) => {
  try {
    ...
    if (
      ...
      console.log(expense.expenseOwner, expense.groupId);
      console.log(ownerValidation);
      if (!ownerValidation) {
        var err = new Error("Please provide a valid group owner");
        err.status = 400;
...
This function is used to deleted the expense added to the group
Accepts: Group ID not null group ID exist in the DB 
...
This function is used to return the expense spend on each category in a group 
Accepts : groupID 
...
//       data: expense,
//     });
//     res.render("report", { expense: expense });
//   } catch (err) {
//
//     res.status(err.status || 500).json({
//       message: err.message,
